...
| **USB 连接 (ADB)** | 用数据线连接电脑，电脑端经由 adb 转发与手机通信，无需局域网，延迟更低 |
...
| **外接手柄转发** | 通过 OTG 或蓝牙连接实体手柄，基于 SDL3 + HIDAPI，支持多手柄，与触屏操作一起转发 |
| **体感操控** | 支持手机/外接手柄的陀螺仪与加速度计，可映射为鼠标/摇杆/手柄输入，支持死区/反死区与加速度计转向 |
| **键盘和鼠标** | 完整键盘映射和鼠标控制，触摸板支持多点触控、滚轮与 7 种可自定义手势 |
| **音量键映射** | 支持为手机音量键设置组合键 |
...
| **外观定制** | 背景、按钮、摇杆、触摸板等均支持纯色或图片自定义，可导出为 ZIP 配置，颜色可绑定手柄 LED |
...
| **手柄音频/音圈马达** | 实体 DualSense 扬声器播放 PC 音频，支持音圈马达震动 |
...
电脑端会优先使用正在运行的 adb，其次使用 PATH / Android SDK 中的 adb，最后使用程序目录 `platform-tools` 下的内置 adb。检测到 USB 设备后点击「连接」即可。
...
- USB 模式需要电脑端具备 adb（Android SDK platform-tools）；也可把 `platform-tools` 放到电脑端程序目录作为内置 adb
