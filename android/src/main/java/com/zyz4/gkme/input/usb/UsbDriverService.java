...
public class UsbDriverService extends Service implements UsbDriverListener {
    ...
    private static final String ACTION_USB_PERMISSION =
            "com.zyz4.gkme.USB_PERMISSION";

    private UsbManager usbManager;
    /** Claim every matching USB device, even when the kernel already exposes it as an input device. */
    private volatile boolean bindAllUsb = true;
        ...
        if (shouldClaimDevice(device, bindAllUsb)) {
            ...
            if (!usbManager.hasPermission(device)) {
                ...
                try {
                    ...
                    Toast.makeText(this, "USB 设备被系统策略禁止访问", Toast.LENGTH_LONG).show();
                    if (stateListener != null) {
                        stateListener.onUsbPermissionPromptCompleted();
                    }
                }
