...
public class DualSenseController extends AbstractDualSenseController {
   ...
   private static final int[] SUPPORTED_PRODUCTS_ALT = {
           0x0CF2,//PS5 DualSense (alternative PID - firmware variant)
   };
   ...
   public static boolean canClaimDevice(UsbDevice device) {
      for (int supportedVid : SUPPORTED_VENDORS) {
         ...
         for (int supportedPid:SUPPORTED_PRODUCTS_ALT) {
            if (device.getVendorId() == supportedVid
                    && device.getProductId()==supportedPid
                    && device.getInterfaceCount() >= 1) {
               return true;
...
//      LimeLog.info("axi->gyroy:"+gyroY);
   ...
   protected void onAdvancedAudioHapticsStarted() {
      // The DualSense audio route/volume is configured once in doInit(). Re-applying it
      // here was observed to disturb the controller, so it is intentionally left out.
   }
